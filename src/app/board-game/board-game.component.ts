import { Component, DoCheck, ElementRef, Input, OnInit, Output, ViewChild, EventEmitter, Renderer2 } from '@angular/core';
...
export class BoardGameComponent implements OnInit, DoCheck {
  ...
  ngOnInit(): void {
    ...
    for (let i = 0; i < arraySlice.length; i++) {
      this.newArray.push(arraySlice[i]);
      this.newArray.push({
        imagen: arraySlice[i].imagen,
        id: String(Number(arraySlice[i].id)*100)
      });
    }
  ...
  fisherYatesShuffle(array: any) {
    for (var i = array.length - 1; i > 0; i--) {
      var j = Math.floor(Math.random() * (i + 1)); // devuelve un indice random
      [array[i], array[j]] = [array[j], array[i]]; // Aquí ocurre el swap de items en el array
    }
  }
    ...
    if (!this.loseGame) {
      ...
      if (this.flipCardCount == 0) {
        ...
        for (let i = 0; i < getCards.length; i++) {
          this.renderer.setStyle(getCards[i], 'pointerEvents', 'none');
        }
        setTimeout(() => {
          this.checkMatch();
        }, 1000);
  ...
  checkMatch() {
    ...
    for (let i = 0; i < getCards.length; i++) {
      if (this.isMatched == this.newArrayLength) {
        this.renderer.setStyle(getCards[i], 'pointerEvents', 'none');
        this.renderer.setStyle(getCards[i].children[1],
          'backgroundcolor',
          'lawngreen');
      } else if (!getCards[i].classList.contains('match')) {
        this.renderer.setStyle(getCards[i], 'pointerEvents', 'auto');
      }
    }
  }
}